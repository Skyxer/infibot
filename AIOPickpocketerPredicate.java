...
public class AIOPickpocketer extends Script {
  ...
  // Script vars
	...
	private static double[] NPC_XP = { 8, 8, 14.5, 26, 43, 46.5, 84.3, 151.75,
			273.3, 353.3 };
	private String npcChoiceString;
	// Paint vars
	...
	public boolean getPickpocketNpc() {
		Npc npc = Npcs.getNearest(new Predicate<Npc>() {
			@Override
			public boolean accept(Npc npc) {
				return npc.getName().equals(npcChoiceString)
						&& Camera.isVisible(npc);
			}
		});
		if (npc != null) {
			return npc.interact("Pickpocket");
		}
		return false;
	}
	...
	public int loop() {
		...
		if (!Players.getMyPlayer().isMoving()
			...
			pickpocketNpc(getNpcId(npcChoiceString));
		}
	...
	public boolean onStart() {
		npcChoiceString = (String) JOptionPane.showInputDialog(null,
